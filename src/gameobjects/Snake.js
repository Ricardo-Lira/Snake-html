class Snake {
    constructor(scene){
        ...
        //Genera el cuerpo
        for (let i = 0; i < 3; i++) {
            this.cuerpo.push(this.scene.physics.add.image(100 + i * 10 , 100, 'cuerpo')
                        .setOrigin(0))
            
        }

        //Genera las coliciones
        for (let i = 1; i < 10; i++) {
    ...
    crece(){
        const objeto = this.cuerpo[this.cuerpo.length -1];
        const newObjeto = this.scene.physics.add.image(objeto.x, objeto.y ,'cuerpo').setOrigin(0);

        this.cuerpo.push(newObjeto);
        this.scene.physics.add.collider(this.cuerpo[0], newObjeto, ()=>
        this.choca());
    }
    choca(){
